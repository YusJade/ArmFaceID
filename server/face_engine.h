...
class FaceEngine {
 public:
  ...
  std::vector<SeetaPointF> DetectPoints(const SeetaImageData &image,
  ...
  std::vector<SeetaPointF> DetectPoints(const SeetaImageData &image,
                                        const SeetaFaceInfo &face) const {
  ...
  float Compare(const SeetaImageData &image1, const SeetaRect &face1,
  ...
  float Compare(const SeetaImageData &image1, const SeetaFaceInfo &face1,
                const SeetaImageData &image2,
                const SeetaFaceInfo &face2) const {
    return this->Compare(image1, face1.pos, image2, face2.pos);
  }

  float Compare(const SeetaImageData &image1,
                const SeetaImageData &image2) const {
  ...
  int64_t Query(const SeetaImageData &image,
                float *similarity = nullptr) const {
    auto faces = this->DetectFaces(image);
    if (faces.empty()) return -1;
    return this->Query(image, faces[0], similarity);
  }

  int64_t Query(const SeetaImageData &image, const SeetaFaceInfo &face,
                float *similarity = nullptr) const {
    return this->Query(image, face.pos, similarity);
  }

  int64_t Query(const SeetaImageData &image, const SeetaRect &face,
                float *similarity = nullptr) const {
    auto points = DetectPoints(image, face);
    return FDB.Query(image, points.data(), similarity);
  }

  size_t QueryTop(const SeetaImageData &image, size_t N, int64_t *index,
  ...
  size_t QueryTop(const SeetaImageData &image, const SeetaFaceInfo &face,
                  size_t N, int64_t *index, float *similarity) const {
    return this->QueryTop(image, face.pos, N, index, similarity);
  }

  size_t QueryTop(const SeetaImageData &image, const SeetaRect &face, size_t N,
                  int64_t *index, float *similarity) const {
    auto points = this->DetectPoints(image, face);
    return FDB.QueryTop(image, points.data(), N, index, similarity);
  }

  float Compare(const SeetaImageData &image1, const SeetaPointF *points1,
                const SeetaImageData &image2,
                const SeetaPointF *points2) const {
  ...
  int Delete(int64_t index) { return FDB.Delete(index); }

  void Clear() { FDB.Clear(); }

  size_t Count() const { return FDB.Count(); }

  int64_t Query(const SeetaImageData &image, const SeetaPointF *points,
                float *similarity = nullptr) const {
    return FDB.Query(image, points, similarity);
  }

  size_t QueryTop(const SeetaImageData &image, const SeetaPointF *points,
                  size_t N, int64_t *index, float *similarity) const {
    return FDB.QueryTop(image, points, N, index, similarity);
  }

  bool Save(const char *path) const { return FDB.Save(path); }

  bool Load(const char *path) { return FDB.Load(path); }

  bool Save(StreamWriter &writer) const { return FDB.Save(writer); }
  bool Load(StreamReader &reader) { return FDB.Load(reader); }
